...
const GameSelection = () => {
  ...
  const { sports, setSports } = useUserOnboarding();
  ...
  const renderItem = useCallback(
    ({ item }: { item: Sport }) => {
      const isSelected = sports.includes(item.sport);
      return (
        <TouchableOpacity
          onPress={() => toggleSelection(item.sport)}
          className="mb-6 mr-3 w-[48%] rounded-xl overflow-hidden"
          accessibilityLabel={`Select ${item.sport}`}
          accessibilityRole="button"
        >
          <View
            className={`rounded-xl overflow-hidden ${
              isSelected ? "border-4 border-green-600" : "border border-gray-200"
            } bg-gray-100`}
          >
            <Image
              resizeMode="cover"
  ...
  return (
    ...
    >
      ...
      <Text className="mb-4">Select your favorite sport to get started</Text>
      ...
      <View className="absolute bottom-6 left-6 right-6">
        <TouchableOpacity
          className={`py-4 rounded-xl ${
            sports.length > 0 ? "bg-green-600" : "bg-slate-600 opacity-50"
          }`}
          disabled={sports.length === 0}
          onPress={handleDebut}
          accessibilityLabel="Proceed to sign up"
          accessibilityRole="button"
        >
          <Text className="text-white text-center font-bold text-base">
            {sports.length > 0 ? "Make My Debut" : "Please select a sport"}
          </Text>
        </TouchableOpacity>
      </View>
    </View>
